...
**The reference implementation lives at [`packages/connectors/hello-react/`](../../packages/connectors/hello-react/).** It's a real Cloudflare Worker that builds, deploys, and serves a valid HTML response with the workspace's design tokens. Read its [README](../../packages/connectors/hello-react/README.md) before reading the rest of this section — the README is the source of truth.

The summary here is for context; the connector itself is the recipe.

### What hello-react demonstrates

- Worker entry uses `defineGuestApp` + `createGuestWorker` (same as any other guest)
- Iframe content is a **React app**, bundled separately with esbuild
- Components imported directly from `@ensemble-edge/ui`: `Card`, `Table`, `Button`
- Tailwind v4 picks up workspace's design tokens via `@import '@ensemble-edge/ui/globals.css'`
- Bundles inlined into the worker via wrangler's `[[rules]]` Text loader — no `?raw`, no `fs.readFileSync`, no custom codegen
- The HTML response links `/_ensemble/brand/css` so brand changes propagate without redeploy

### Verified bundle profile

Measured in the connector's actual build, 2026-05-11:

| Asset | Size | Gzipped |
|---|---|---|
| JS bundle (React 18 + Radix + workspace UI + app code) | 404 KB | **125 KB** |
| CSS bundle (workspace tokens + utility classes) | 56 KB | **9 KB** |
| Combined inline HTML response | 470 KB | ~135 KB |

Well under CF Workers' 1 MB compressed limit. React + React-DOM measured as exactly one resolved copy each — peer-dep dedup works as designed.

### Build flow (from the connector's `package.json`)

```
src/styles.css  → tailwindcss → dist/app.bundle.css
src/app.tsx     → esbuild     → dist/app.bundle.js
                                              │
                                              ▼
                                        src/index.ts (worker)
                                        ...
                                        import bundleCss from '../dist/app.bundle.css'
                                              ...
                                              ▼
                                        wrangler deploy
                                        (Text rules in wrangler.toml resolve those imports to strings)
```
...
The hello-react README has a [step-by-step debug list](../../packages/connectors/hello-react/README.md#debugging-order-if-it-doesnt-render). Use it instead of guessing.
