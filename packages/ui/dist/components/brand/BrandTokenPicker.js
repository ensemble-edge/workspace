...
function isGradientRef(v) {
    return /^gradient-[a-z0-9-]+$/.test(v);
}
function gradientSlug(v) {
    return v.replace(/^gradient-/, '');
}
function resolveDisplay(value, palettes, gradients) {
    if (value === 'auto')
        return { label: 'auto', swatch: 'transparent', isGradient: false, isToken: false };
    if (value === 'white')
        return { label: 'white', swatch: '#ffffff', isGradient: false, isToken: true };
    if (value === 'black')
        return { label: 'black', swatch: '#000000', isGradient: false, isToken: true };
    if (isRungRef(value)) {
    ...
    if (isHex(value))
        return { label: value.toUpperCase(), swatch: value, isGradient: false, isToken: false };
    return { label: value, swatch: '#000000', isGradient: false, isToken: false };
}
export function BrandTokenPicker({ value, onChange, palettes, allowHex = false, allowAuto = false, allowSystem = false, allowGradient = false, gradients = [], allowedRoles = ALL_ROLES, label, className, }) {
    ...
    return (_jsxs(Popover, { children: [_jsx(PopoverTrigger, { asChild: true, children: _jsxs("button", { type: "button", "aria-label": label ?? `Pick color token (current: ${display.label})`, className: cn('inline-flex items-center gap-2 px-2 py-1.5 rounded-md border border-border bg-background', 'hover:bg-muted/50 outline-none focus-visible:ring-2 focus-visible:ring-foreground/30 transition-colors', 'text-left min-w-0', className), children: [_jsx("span", { className: "w-4 h-4 rounded border border-black/10 shrink-0", style: swatchStyle }), _jsx("span", { className: cn('font-mono text-xs truncate', display.isToken ? 'text-foreground' : 'text-muted-foreground'), children: display.label })] }) }), _jsxs(PopoverContent, { align: "start", className: "w-[320px] p-3 max-h-[420px] overflow-y-auto", children: [(allowAuto || allowSystem) && (_jsxs("div", { className: "flex flex-wrap gap-1.5 mb-3 pb-3 border-b", children: [allowAuto && (_jsx(TokenChip, { active: value === 'auto', label: "auto", swatch: "transparent", onClick: () => onChange('auto'), title: "APCA-picked at render time" })), allowSystem && (_jsxs(_Fragment, { children: [_jsx(TokenChip, { active: value === 'white', label: "white", swatch: "#ffffff", onClick: () => onChange('white') }), _jsx(TokenChip, { active: value === 'black', label: "black", swatch: "#000000", onClick: () => onChange('black') })] }))] })), _jsx("div", { className: "space-y-2", children: allowedRoles.map((role) => (_jsxs("div", { className: "space-y-1", children: [_jsx("p", { className: "text-[10px] uppercase tracking-[0.08em] text-muted-foreground font-medium", children: role }), _jsx("div", { className: "grid grid-cols-5 gap-1", children: ALL_RUNGS.map((rung) => {
                                    ...
                                    }) })] }, role))) }), allowGradient && gradients.length > 0 && (_jsxs("div", { className: "mt-3 pt-3 border-t space-y-1", children: [_jsx("p", { className: "text-[10px] uppercase tracking-[0.08em] text-muted-foreground font-medium", children: "gradients" }), _jsx("div", { className: "grid grid-cols-1 gap-1", children: gradients.map((g) => {
                                    const ref = `gradient-${g.slug}`;
                                    const isSelected = value === ref;
                                    return (_jsxs("button", { type: "button", "aria-label": ref, onClick: () => onChange(ref), className: cn('flex items-center gap-2 p-1.5 rounded outline-none text-left', 'hover:bg-muted/50 focus-visible:ring-2 focus-visible:ring-foreground/30 transition-colors', isSelected && 'ring-2 ring-foreground/40 bg-muted/30'), children: [_jsx("span", { className: "h-6 w-16 rounded border border-black/10 shrink-0", style: { background: g.css } }), _jsxs("span", { className: "flex flex-col min-w-0", children: [_jsx("span", { className: "text-xs font-medium text-foreground truncate", children: g.name }), _jsx("span", { className: "font-mono text-[9px] text-muted-foreground truncate", children: ref })] })] }, g.slug));
                                }) })] })), allowHex && (_jsxs("div", { className: "mt-3 pt-3 border-t", children: [_jsx("p", { className: "text-[10px] uppercase tracking-[0.08em] text-muted-foreground font-medium mb-2", children: "Custom hex" }), _jsx(ColorPickerPanel, { value: isHex(value) ? value : '#000000', onChange: (hex) => onChange(hex) })] }))] })] }));
}
