...
function PaletteFaceContent({ role, palette, mode, onNameChange, fg, }) {
    return (_jsxs("div", { className: "flex flex-col gap-1", children: [mode === 'edit' && onNameChange ? (_jsx(Input, { value: palette.name, onChange: (e) => onNameChange(e.currentTarget.value), onClick: (e) => e.stopPropagation(), className: "bg-transparent border-0 px-0 h-auto text-[26px] font-normal leading-[1.05] tracking-tight focus-visible:ring-0 hover:bg-black/5", style: { color: fg } })) : (_jsx("p", { className: "text-[26px] font-normal leading-[1.05] tracking-tight", style: { color: fg }, children: palette.name })), _jsx("p", { className: "text-[10px] font-medium uppercase tracking-[0.08em] opacity-80", style: { color: fg }, children: role })] }));
}
function PaletteFaceFooter({ role, hex, fg }) {
    return (_jsxs("div", { className: "flex items-end justify-between font-mono text-[11px]", style: { color: fg }, children: [_jsxs("span", { className: "opacity-90", children: [role, "-main"] }), _jsx("span", { className: "opacity-90", children: hex.toUpperCase() })] }));
}
function RungChip({ role, rung, hex, label, mode, onOverride, showMeta = true }) {
    if (mode === 'edit' && onOverride) {
        return (_jsxs(Popover, { children: [_jsx(PopoverTrigger, { asChild: true, children: _jsxs("button", { type: "button", "aria-label": `Edit ${role}-${rung}`, className: "flex flex-col gap-1 text-left outline-none focus-visible:ring-2 focus-visible:ring-foreground/30 rounded", children: [_jsx("span", { className: "h-[24px] rounded-md border-[0.5px] border-black/10 cursor-pointer hover:-translate-y-px transition-transform", style: { backgroundColor: hex } }), showMeta && (_jsxs("span", { className: "flex flex-col gap-0", children: [_jsx("span", { className: "text-[11px] font-medium text-foreground", children: label }), _jsx("span", { className: "font-mono text-[9.5px] text-muted-foreground", children: hex.toUpperCase() })] }))] }) }), _jsx(PopoverContent, { className: "p-3 w-[280px]", align: "start", children: _jsx(ColorPicker, { label: `${role}-${rung} override`, description: "Leave as-is to use the OkLCh-derived value.", value: hex, onChange: (next) => onOverride(rung, next), onReset: () => onOverride(rung, null) }) })] }));
    }
    return (_jsxs("div", { className: "flex flex-col gap-1", children: [_jsx(Swatch, { color: hex, label: hex.toUpperCase(), className: "h-[24px] rounded-md" }), showMeta && (_jsxs(_Fragment, { children: [_jsx("span", { className: "text-[11px] font-medium text-foreground", children: label }), _jsx("span", { className: "font-mono text-[9.5px] text-muted-foreground", children: hex.toUpperCase() })] }))] }));
}
function NeutralStrip({ palette, resolved, mode, onNameChange, onRungOverride }) {
    const isEdit = mode === 'edit';
    return (_jsx("div", { className: "rounded-2xl border-[0.5px] border-black/[0.07] bg-background p-5", children: _jsxs("div", { className: "grid grid-cols-[180px_1fr] gap-6 items-center", children: [_jsxs("div", { className: "flex flex-col gap-1", children: [isEdit && onNameChange ? (_jsx(Input, { value: palette.name, onChange: (e) => onNameChange(e.currentTarget.value), className: "bg-transparent border-0 px-0 h-auto text-[20px] font-normal tracking-tight focus-visible:ring-0 hover:bg-black/5" })) : (_jsx("p", { className: "text-[20px] font-normal tracking-tight", children: palette.name })), _jsx("p", { className: "text-[11px] font-medium uppercase tracking-[0.08em] text-muted-foreground", children: "neutral" }), _jsx("p", { className: "text-xs text-muted-foreground", children: "Surfaces \u00B7 borders \u00B7 muted text. Derived from primary by default." })] }), _jsx("div", { className: "grid grid-cols-5 gap-2", children: ['dark', 'main', 'bright', 'pastel', 'faded'].map((rung) => (_jsxs("div", { className: "flex flex-col gap-1", children: [rung === 'main' ? (_jsx(Swatch, { color: resolved.main, label: resolved.main.toUpperCase(), className: "h-[32px] rounded-md", style: { boxShadow: '0 0 0 1.5px var(--foreground, #18181B)' } })) : (_jsx(RungChip, { role: "neutral", rung: rung, hex: resolved[rung], label: rung.charAt(0).toUpperCase() + rung.slice(1), mode: mode, onOverride: onRungOverride, showMeta: false })), _jsx("span", { className: "text-[11px] font-medium text-foreground capitalize", children: rung }), _jsx("span", { className: "font-mono text-[9.5px] text-muted-foreground", children: resolved[rung].toUpperCase() })] }, rung))) })] }) }));
}
function GradientBanner({ gradient, onColor, mode, onNameChange }) {
    ...
    return (_jsxs("div", { className: "rounded-2xl overflow-hidden border-[0.5px] border-black/[0.07] bg-background", children: [_jsx("div", { role: "button", tabIndex: 0, "aria-label": `Copy gradient ${gradient.slug}`, onClick: () => copyToClipboard(css, `gradient-${gradient.slug}`), onKeyDown: (e) => {
                ...
                }, className: "h-[90px] flex items-center px-5 cursor-pointer transition-[filter] hover:brightness-[0.97] outline-none focus-visible:ring-2 focus-visible:ring-foreground/30", style: { background: css, color: onColor.hex }, children: isEdit && onNameChange ? (_jsx(Input, { value: gradient.name, onChange: (e) => onNameChange(e.currentTarget.value), onClick: (e) => e.stopPropagation(), className: "bg-transparent border-0 px-0 h-auto text-[28px] font-normal tracking-tight focus-visible:ring-0 hover:bg-black/5", style: { color: onColor.hex } })) : (_jsx("p", { className: "text-[28px] font-normal tracking-tight", style: { color: onColor.hex }, children: gradient.name })) }), _jsxs("div", { className: "flex items-center justify-between px-4 py-2.5 text-[11px]", children: [_jsxs("span", { className: "font-mono text-muted-foreground", children: ["gradient-", gradient.slug] }), _jsx("div", { className: "flex items-center gap-2 flex-wrap", children: gradient.resolvedStops.map((s, i) => (_jsxs(React.Fragment, { children: [i > 0 && _jsx("span", { className: "text-muted-foreground", children: "\u2192" }), _jsxs("span", { className: "inline-flex items-center gap-1.5", children: [_jsx("span", { className: "w-[14px] h-[14px] rounded border-[0.5px] border-black/10", style: { backgroundColor: s.hex } }), _jsx("span", { className: "font-mono text-muted-foreground", children: colorLabel(s.token, s.hex).primary })] })] }, i))) }), _jsx("span", { className: "font-mono text-muted-foreground bg-black/5 px-2 py-0.5 rounded", children: gradient.mode === 'radial' ? 'radial' : `linear · ${gradient.angle}°` })] })] }));
}
const SEM_LABEL = {
    success: 'Success',
    info: 'Info',
