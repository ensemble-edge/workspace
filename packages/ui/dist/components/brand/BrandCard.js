...
function NeutralStrip({ palette, resolved, mode, onNameChange, onMainChange, onHueModeChange, onRungOverride }) {
    const isEdit = mode === 'edit';
    const hueMode = palette.hueMode ?? 'branded';
    return (_jsx("div", { className: "rounded-2xl border-[0.5px] border-black/[0.07] bg-background p-5", children: _jsxs("div", { className: "grid grid-cols-[200px_1fr] gap-6 items-start", children: [_jsxs("div", { className: "flex flex-col gap-2", children: [isEdit && onNameChange ? (_jsx(Input, { value: palette.name, onChange: (e) => onNameChange(e.currentTarget.value), className: "bg-transparent border-0 px-0 h-auto text-[22px] font-normal tracking-tight focus-visible:ring-0 hover:bg-black/5", style: { fontFamily: 'var(--brand-font-display, var(--brand-font-heading, inherit))' } })) : (_jsx("p", { className: "text-[22px] font-normal tracking-tight leading-none", style: { fontFamily: 'var(--brand-font-display, var(--brand-font-heading, inherit))' }, children: palette.name })), _jsx("p", { className: "text-[11px] font-medium tracking-[0.12em] text-muted-foreground", style: {
                                textTransform: 'lowercase',
                                fontFamily: 'var(--brand-font-eyebrow, var(--brand-font-body, inherit))',
                            }, children: "neutral" }), isEdit && onHueModeChange ? (_jsxs("div", { className: "flex flex-col gap-1.5 mt-1", children: [_jsx("span", { className: "text-[10px] font-medium text-muted-foreground uppercase tracking-[0.04em]", children: "Hue" }), _jsx("div", { className: "flex flex-wrap gap-1", children: ['branded', 'warm', 'cool', 'true', 'custom'].map((m) => (_jsx("button", { type: "button", onClick: () => onHueModeChange(m), className: cn('inline-flex items-center px-2 py-1 rounded-md border text-[10px] font-medium transition-colors', 'outline-none focus-visible:ring-2 focus-visible:ring-foreground/30', hueMode === m
                                            ? 'border-foreground/40 bg-foreground/5 text-foreground'
                                            : 'border-border bg-background text-muted-foreground hover:text-foreground hover:border-foreground/20'), children: HUE_MODE_LABELS[m] }, m))) }), _jsxs("p", { className: "text-[10px] text-muted-foreground leading-relaxed mt-1", children: [hueMode === 'branded' && 'Inherits primary\'s hue. Cohesive with the brand.', hueMode === 'warm' && 'Amber/sand tint. Use for sandstone / cream neutrals.', hueMode === 'cool' && 'Blue-grey tint. Pairs with cooler primaries.', hueMode === 'true' && 'Pure achromatic. No hue.', hueMode === 'custom' && 'Click the Main chip to pick any hex.'] })] })) : (_jsxs("p", { className: "text-[10px] text-muted-foreground mt-0.5", children: ["hue \u00B7 ", HUE_MODE_LABELS[hueMode].toLowerCase()] }))] }), _jsx("div", { className: "grid grid-cols-5 gap-2", children: ['dark', 'main', 'bright', 'pastel', 'faded'].map((rung) => (_jsxs("div", { className: "flex flex-col gap-1", children: [rung === 'main' ? (
                            ...
                            isEdit && onMainChange ? (_jsxs(Popover, { children: [_jsx(PopoverTrigger, { asChild: true, children: _jsx("button", { type: "button", "aria-label": "Edit neutral main", className: "rounded-md border-[0.5px] border-black/10 cursor-pointer hover:-translate-y-px transition-transform outline-none focus-visible:ring-2 focus-visible:ring-foreground/30", style: {
                                                height: 36,
                                                backgroundColor: resolved.main,
                                                boxShadow: '0 0 0 1.5px var(--foreground, #18181B)',
                                            } }) }), _jsx(PopoverContent, { className: "p-3 w-[280px]", align: "start", children: _jsx(ColorPickerPanel, { label: "Neutral main", description: hueMode === 'custom'
                                                ? 'Operator-typed neutral. Other rungs derive from this.'
                                                : 'Picking a hex switches Hue to Custom.', value: resolved.main, onChange: (hex) => {
                                                onMainChange(hex);
                                                // Picking a hex implicitly means "Custom" mode
                                                if (hueMode !== 'custom')
                                                    onHueModeChange?.('custom');
                                            } }) })] })) : (_jsx(Swatch, { color: resolved.main, label: resolved.main.toUpperCase(), className: "rounded-md", style: {
                                    height: 36,
                                    boxShadow: '0 0 0 1.5px var(--foreground, #18181B)',
                                } }))) : (_jsx(RungChip, { role: "neutral", rung: rung, hex: resolved[rung], label: rung.charAt(0).toUpperCase() + rung.slice(1), mode: mode, onOverride: onRungOverride, showMeta: false, chipHeight: 36 })), _jsx("span", { className: "text-[10px] font-medium text-foreground capitalize tracking-[0.04em]", style: { fontFamily: 'var(--brand-font-label, var(--brand-font-body, inherit))' }, children: rung }), _jsx("span", { className: "text-[9.5px] text-muted-foreground", style: { fontFamily: 'var(--brand-font-mono, ui-monospace, SFMono-Regular, Menlo, monospace)' }, children: resolved[rung].toUpperCase() })] }, rung))) })] }) }));
}
