...
function resolveSegmentColor(raw, palettes, gradients) {
    if (!raw)
        return {};
    const t = raw.trim();
...
export const Wordmark = React.forwardRef(({ segments, imageUrl, name, alt, imageHeight = 32, surfaceColor, palettes, gradients, className, ...rest }, ref) => {
    ...
    if (segments && segments.length > 0) {
        ...
        return (_jsx("span", { ref: ref, className: cn("inline-flex items-baseline", className), style: wrapStyle, "aria-label": name, ...rest, children: segments.map((s, i) => {
                ...
                return (_jsx("span", { style: Object.keys(finalStyle).length ? finalStyle : undefined, children: s.text }, i));
            }) }));
    }
    if (imageUrl) {
