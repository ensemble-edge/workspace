 ...
 * can read appId from the pathname. Cached on first call — the appId
 * doesn't change for the iframe's lifetime.
 */
let _resolvedAppId: string | null = null;
function resolveAppId(): string {
  if (_resolvedAppId) return _resolvedAppId;
  const m = window.location.pathname.match(/^\/_ensemble\/apps\/([^/]+)/);
  if (!m) throw new Error('useSecret: could not resolve appId from iframe URL');
  _resolvedAppId = m[1]!;
  return _resolvedAppId;
}

function useSecret({ key, scope = 'app' }: UseSecretOptions): UseSecretResult {
  const [loading, setLoading] = React.useState(false);
  const [error, setError] = React.useState<string | null>(null);

  const baseUrl = React.useMemo(() => {
    const appId = resolveAppId();
    return `/_ensemble/apps/${encodeURIComponent(appId)}/_secrets/${encodeURIComponent(key)}?scope=${scope}`;
  }, [key, scope]);
