...
import { getCredential, getWorkspacePublicUrl } from './credentials';
 ...
 */
export async function issueInvite(env: Env, input: IssueInviteInput): Promise<IssueInviteResult> {
  ...
  if (input.revokePrior) {
    const prior = await env.DB.prepare(
      `SELECT invite_token FROM users WHERE id = ?`,
    ).bind(input.userId).first<{ invite_token: string | null }>();
  ...
  await env.DB.prepare(
    `UPDATE users
        SET invite_pending = 1,
            ...
            invite_token = ?
      WHERE id = ?`,
  ).bind(invitedAt, input.inviterUserId, token, input.userId).run();
 ...
 * "send failed".
 */
export async function isEmailReady(env: Env, workspaceId: string): Promise<boolean> {
