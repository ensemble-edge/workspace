...
export async function getCredential(
  ...
  const row = await env.DB.prepare(
    `SELECT key, category, is_secret, value_encrypted, value_plain, updated_at, updated_by
       FROM workspace_credentials
      WHERE workspace_id = ? AND key = ?`,
  ).bind(workspaceId, key).first<DbRow>();
...
export async function setCredential(
  ...
  await env.DB.prepare(
    ...
    workspaceId, key, category, isSecret, encrypted, plain,
    opts.updatedBy ?? null,
  ).run();
}
...
export async function deleteCredential(
  ...
  await env.DB.prepare(
    `DELETE FROM workspace_credentials WHERE workspace_id = ? AND key = ?`,
 ...
 */
export async function getWorkspacePublicUrl(
  env: Env,
  workspaceId: string,
  ...
  if (fallbackRequest) {
    ...
    console.warn(
      `[credentials] workspace_public_url not set; falling back to request origin ${fallback}. ` +
