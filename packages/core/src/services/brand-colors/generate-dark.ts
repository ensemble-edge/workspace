...
import {
  isPaletteRungRef, parseRungRef,
  type BrandColorsDoc, type Theme, type ThemeBindings, type ThemeBindingValue,
  type PaletteRole, type PaletteRungRef,
} from './schema';
 ...
 */
export function generateDarkTheme(doc: BrandColorsDoc): GenerateResult {
  ...
  if (brandResult.warning) warnings.push({ binding: 'brand', ...brandResult.warning });

  const brandBgResult = preserveContrast(light['brand-bg'], palettes, lightCanvasHex, darkCanvasHex);
  if (brandBgResult.warning) warnings.push({ binding: 'brand-bg', ...brandBgResult.warning });

  const borderResult = preserveContrast(light.border, palettes, lightCanvasHex, darkCanvasHex);
  if (borderResult.warning) warnings.push({ binding: 'border', ...borderResult.warning });
 ...
 */
function invertCanvasToDark(lightHex: string): string {
 ...
 */
function liftLuminance(hex: string, dL: number): string {
  const p = oklch(hex);
  if (!p) return hex;
...
/* ──────────────────────────────────────────────────────────────
 * Phase 2 — contrast-preserving brand bindings
 * ──────────────────────────────────────────────────────────── */

interface PreserveResult {
  value: ThemeBindingValue;
  warning?: Omit<BindingWarning, 'binding'>;
}
 ...
 */
function preserveContrast(
  lightValue: ThemeBindingValue,
  palettes: ResolvedPalettes,
  lightCanvasHex: string,
  darkCanvasHex: string,
): PreserveResult {
  if (lightValue === 'auto') return { value: 'auto' };
  if (!isPaletteRungRef(lightValue)) return { value: lightValue };
  ...
  return {
    value: `${ref.role}-${pick.rung}` as PaletteRungRef,
    warning,
  };
}
 ...
 */
export function autoForeground(canvasHex: string): string {
 ...
 */
export function generateLightFromDark(doc: BrandColorsDoc): GenerateResult | null {
  if (!doc.themes.dark) return null;
  const palettes = resolvePalettes(doc);
  ...
  if (brand.warning)   warnings.push({ binding: 'brand', ...brand.warning });
  if (brandBg.warning) warnings.push({ binding: 'brand-bg', ...brandBg.warning });
  if (border.warning)  warnings.push({ binding: 'border', ...border.warning });
