...
interface RungOffset {
  /** Absolute L target relative to Main; e.g. -0.18 means 0.18 below. */
  dL: number;
  /** Chroma multiplier — 1.0 preserves saturation; < 1.0 desaturates. */
  cMul: number;
}

const RUNG_OFFSETS: Record<Exclude<RungName, 'main'>, RungOffset> = {
 ...
 */
export function deriveRung(mainHex: string, rung: Exclude<RungName, 'main'>): string {
  const parsed = oklch(mainHex);
  if (!parsed) return mainHex;
 ...
 */
export function deriveRungs(mainHex: string): Record<Exclude<RungName, 'main'>, string> {
  return {
 ...
 */
export function hueBiasedForeground(bgHex: string, prefer: 'light' | 'dark'): string {
  const p = oklch(bgHex);
  if (!p) return prefer === 'light' ? '#FAFAFA' : '#0A0A0A';
