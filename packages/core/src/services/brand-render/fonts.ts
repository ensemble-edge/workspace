 ...
 */
export function googleFontR2Key(family: string, weight: number): string {
 ...
 */
export function customFontR2Key(workspaceSlug: string, family: string, weight: number): string {
  const slug = family.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '');
 ...
 */
export function resolveFontR2Key(family: string, weight: number): string {
...
export async function loadFonts(
  ...
  for (const req of requests) {
    ...
    if (!response) {
      ...
      if (!obj) {
        throw new FontNotInR2Error(req.family, req.weight as FontData['weight'], key);
      }
      ...
      try { await cache.put(cacheKey, response.clone()); } catch { /* noop */ }
    }
    ...
    out.push({
      name: req.family,
      data,
      weight: (req.weight as FontData['weight']),
...
export class FontNotInR2Error extends Error {
  constructor(public family: string, public weight: FontData['weight'], public r2Key: string) {
