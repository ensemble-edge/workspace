 ...
 */
export const ROLE_EXAMPLES: Record<FontRole, string[]> = {
...
export const PALETTE_USAGE: Record<PaletteRole, { usage: string; examples: string[] }> = {
  primary: {
...
export const SEMANTIC_USAGE: Record<SemanticRole, string> = {
 ...
 */
export const DEFAULT_RADIUS: Record<'sm' | 'md' | 'lg' | 'xl' | 'full', string> = {
  sm: '0.25rem',
  md: '0.375rem',
  lg: '0.5rem',
  xl: '0.75rem',
...
export const DEFAULT_SHADOW: Record<'sm' | 'md' | 'lg' | 'xl', string> = {
...
export const COMPONENT_DEFAULTS = {
  button: { radius_ref: 'md' as const, padding_x_ref: 4, padding_y_ref: 2, gap_ref: 2 },
  card:   { radius_ref: 'lg' as const, padding_ref: 6, shadow_ref: 'sm' as const },
  input:  { radius_ref: 'md' as const, padding_x_ref: 3, padding_y_ref: 2 },
  modal:  { radius_ref: 'xl' as const, padding_ref: 8, shadow_ref: 'xl' as const },
  badge:  { radius_ref: 'sm' as const, padding_x_ref: 2, padding_y_ref: 1 },
};

// ─────────────────────────────────────────────────────────────
// Logo variant context (v0.1.93)
// ─────────────────────────────────────────────────────────────
//
// Same idea as ROLE_USAGE for typography: each (composition, finish,
// background) tuple gets a `usage` sentence + `examples` array +
// `recommended_for` hints, so an agent reading a logo variant knows
// WHERE to use it, not just that it exists. The lookup is by
// (composition × finish × background); we fall back gracefully so a
// variant we don't have specific prose for still gets sensible
...
export const COMPOSITION_USAGE: Record<CompositionId, { usage: string; examples: string[] }> = {
  'wordmark-only': {
...
export const FINISH_USAGE: Record<FinishId, { usage: string; examples: string[] }> = {
  'full-color': {
...
export const BACKGROUND_USAGE: Record<BackgroundId, { usage: string; examples: string[] }> = {
  transparent: {
 ...
 */
export const FORMAT_USAGE: Record<string, string> = {
 ...
 */
export const USE_HINT_GUIDANCE: Record<string, string> = {
...
export const DEFAULT_CLEARSPACE = {
  wordmark: { unit: 'x-height' as const, multiplier: 1.0 },
  icon_mark: { unit: 'icon-width' as const, multiplier: 0.25 },
};
...
// ─────────────────────────────────────────────────────────────
// Instructions block (v0.1.93)
// ─────────────────────────────────────────────────────────────
//
// Embedded at the top of every /brand/spec response. Tells a consumer
// (especially an AI agent) HOW to use the spec — not just the data
// structure. The schema documents the format; this documents the
// contract.
//
// Why structured rather than free-form prose:
//   • Each rule has a stable id, so tools can suppress / override /
//     translate / surface a specific rule.
//   • `why` gives the reasoning so the consumer understands intent,
//     not just the directive. Models that understand WHY follow rules
//     in novel situations the rules didn't explicitly cover.
//   • `anti_patterns` is the negative space — equally important.
//   • `for_ai_agents` directly addresses LLMs. Mild prompt-engineering
//     instinct: explicit framing improves compliance.
