...
import { validateBrandDomain, invalidateDomainCache } from '../services/brand-domain';
...
export function createDomainsRoutes(): Hono<AppEnv> {
  ...
  app.get('/_ensemble/domains', async (c) => {
    ...
    const { results } = await c.env.DB.prepare(
      `SELECT domain, proto, verified, created_at FROM workspace_domains
        WHERE workspace_id = ? ORDER BY created_at ASC, domain ASC`,
    )
      .bind(workspace.id)
      .all<{ domain: string; proto: string; verified: number; created_at: string }>();
    return c.json({
  ...
  app.post('/_ensemble/domains', async (c) => {
    ...
    let body: { domain?: string; proto?: string };
    try {
    ...
    if (existing) {
      ...
      return c.json(
        { error: mine ? 'domain already added to this workspace' : 'domain already in use', domain },
        409,
      );
    }
    ...
    await c.env.DB.prepare(
      `INSERT INTO workspace_domains (domain, workspace_id, proto, verified, created_by)
       VALUES (?, ?, ?, 1, ?)`,
    )
  ...
  app.delete('/_ensemble/domains/:domain', async (c) => {
    ...
    if (!res.meta.changes) return c.json({ error: 'not_found', domain }, 404);
