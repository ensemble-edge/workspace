...
export function createWorkspaceContextRoutes(): App {
   ...
   */
  app.get('/_ensemble/workspace/context', async (c: Context<AppEnv>) => {
    const workspace = c.get('workspace');
    if (!workspace?.id) return c.json({ error: 'workspace_not_resolved' }, 400);
  ...
  app.put('/_ensemble/workspace/preferences/locale', async (c) => {
    ...
    const body = await c.req.json<{ locale?: string }>().catch(() => ({ locale: '' as string | undefined }));
    const locale = ((body as { locale?: string }).locale ?? '').trim();
    if (!locale) {
    ...
    const supported = await c.env.DB.prepare(
      `SELECT code FROM workspace_locales WHERE workspace_id = ?`,
    ).bind(workspace.id).all<{ code: string }>().catch(() => ({ results: [] as { code: string }[] }));
