...
const DICT: Record<string, { center: string; updated: string; notFound: string }> = {
...
export interface LegalPageData {
  ...
  toc: Array<{ id: string; title: string; slug: string }>;
}
...
export function renderLegalPage(data: LegalPageData): string {
  ...
  const toc = data.toc
    ...
    .map(
      (e) =>
        `<a href="/legal/${esc(e.slug)}" class="${e.id === data.activeId ? 'active' : ''}">${esc(e.title || e.id)}</a>`,
    )
    .join('');

  const langSwitcher =
    localeSlugs.length > 1
