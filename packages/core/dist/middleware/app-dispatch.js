...
export function appDispatch() {
    return async (c, next) => {
        ...
        try {
            ...
            const { resolveDispatch } = await import('../services/app-dispatch.js');
        ...
        let row = null;
        try {
            row = await c.env.DB.prepare(`SELECT connection_type, binding_name, endpoint_url FROM guest_apps WHERE workspace_id = ? AND id = ?`)
        ...
        if (!row)
            return next(); // no connection info → let normal routing 404
        ...
        if (requestId)
            headers.set(H.REQUEST_ID, requestId);
        if (user) {
            headers.set(H.USER_ID, user.id);
            headers.set(H.USER_EMAIL, user.email);
            const membership = c.get('membership');
            if (membership)
                headers.set(H.USER_ROLE, membership.role);
        }
        ...
        try {
            ...
            if (row.connection_type === 'service_binding' && row.binding_name) {
                const binding = c.env[row.binding_name];
                if (!binding?.fetch) {
...
//# sourceMappingURL=app-dispatch.js.map