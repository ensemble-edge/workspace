 ...
 * @example
 * ```ts
 * import { cloudAuth, DEFAULT_CLOUD_AUTH_HEADERS } from '../index.js';
 *
 ...
 * ```
 */
export function cloudAuth(options) {
    ...
    return async (c, next) => {
        ...
        const userRole = c.req.header(headers.userRole);
 ...
 * @returns Hono middleware handler
 */
export function createCloudAuthMiddleware(config) {
    return cloudAuth({
 ...
 */
async function verifyProxySignature(c, secret, signature, headers) {
    if (!signature) {
        return false;
    }
    try {
