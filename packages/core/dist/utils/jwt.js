 ...
 */
export function getJwtSecret(secret, environment) {
    if (secret && secret.length > 0) {
        return secret;
    }
    // In development, use fallback
    if (!environment || environment === 'development') {
        console.warn('[JWT] Using development fallback secret. Set JWT_SECRET for production!');
        return DEV_SECRET;
    }
 ...
 */
export async function signRefreshToken(sessionId, secret, ttlSeconds) {
    const key = createSecretKey(secret);
    const token = await new SignJWT({
        sid: sessionId,
        type: 'refresh',
