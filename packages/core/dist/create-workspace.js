...
import { cors, workspaceResolver, bootstrapCheck, auth } from './middleware/index.js';
import { runMigrations, migrations } from './db/index.js';
import { createAuthRoutes, createBootstrapRoutes, createGuestGatewayRoutes } from './routes/index.js';
...
export function createWorkspace(config) {
    ...
    app.get('/login', async (c) => {
        const workspace = c.get('workspace');
        const themeMode = await getSavedThemeMode(c.env.DB, workspace?.id || '');
        return c.html(generateLoginHtml(workspace?.name ?? resolvedConfig.workspace.name, resolvedConfig.brand.accent, themeMode));
    });
    // ============================================================================
 ...
 */
function generateLoginHtml(workspaceName, accentColor, themeMode = 'dark') {
    const initialClass = themeMode === 'light' ? '' : 'dark';
    const systemScript = themeMode === 'system' ? `<script>if(window.matchMedia('(prefers-color-scheme:light)').matches)document.documentElement.classList.remove('dark')</script>` : '';
