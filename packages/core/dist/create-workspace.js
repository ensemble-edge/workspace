...
import { cors, publicCors, workspaceResolver, bootstrapCheck, auth, appDispatch } from './middleware/index.js';
import { runMigrations, migrations } from './db/index.js';
import { createAuthRoutes, createBootstrapRoutes, createGuestGatewayRoutes, createGuestSecretsRoutes, createWorkspaceContextRoutes } from './routes/index.js';
...
export function createWorkspace(config) {
    ...
    const brandThemeHandler = async (c) => {
    ...
    const brandTokensPutHandler = async (c) => {
        try {
            const workspace = c.get('workspace');
            if (!workspace?.id) {
    ...
    app.get('/_ensemble/nav', async (c) => {
        ...
        if (workspace?.id) {
            try {
                const { listApps } = await import('./services/app-registry.js');
                const apps = await listApps(c.env, workspace.id);
                const inactive = new Set(apps.filter((a) => a.status !== 'active').map((a) => a.id));
                // Sidebar item id → core app id (only governable, public-facing ones).
                const navIdToApp = { legal: 'core:legal' };
