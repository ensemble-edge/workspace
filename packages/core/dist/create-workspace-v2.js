 ...
 * @example
 * ```ts
 * import { createWorkspace, defineConfig } from './index.js';
 *
 * // Standalone mode
...
import { cors, workspaceResolver, bootstrapCheck, createCloudAuthMiddleware, } from './middleware/index.js';
import { runMigrations, migrations } from './db/index.js';
import { createAuthRoutes, createBootstrapRoutes } from './routes/index.js';
import { registerCoreApps } from './apps/index.js';
import { generateBrandCss } from './apps/core/brand/css.js';
/**
...
function toLegacyConfig(config) {
    const authProviders = config.mode === 'standalone'
        ? config.auth.providers.filter((p) => ['email', 'google', 'github', 'microsoft', 'saml'].includes(p))
        : ['email']; // Default for cloud mode
    return {
...
let SHELL_CSS = '';
try {
    const assets = await import('../../shell/dist/assets.js');
 ...
 */
export function createWorkspaceV2(config) {
...
// ============================================================================
function createStandaloneWorkspace(config) {
    const app = new Hono();
    ...
    app.onError((err, c) => {
        ...
        return c.json({ error: 'internal_error', message: err?.message || 'Internal Server Error', requestId: reqId }, 500);
    });
    // ============================================================================
    ...
    if (config.serveShell) {
        ...
        app.get('/', (c) => {
            ...
            return c.html(generateShellHtml(workspace?.name ?? config.workspace.name, config.brand.accent));
        });
        // Login page
        app.get('/login', (c) => {
            const workspace = c.get('workspace');
            return c.html(generateLoginHtml(workspace?.name ?? config.workspace.name, config.brand.accent));
        });
        // Shell assets
...
// ============================================================================
function createCloudWorkspace(config) {
    const app = new Hono();
    // ============================================================================
...
// ============================================================================
function mountBrandRoutes(app, config) {
    ...
    app.get('/_ensemble/brand/theme', async (c) => {
        ...
        try {
            ...
            const accentResult = await c.env.DB.prepare(`SELECT value FROM brand_tokens
         WHERE workspace_id = ? AND category = 'colors' AND key = 'accent' AND locale = ''`).bind(workspaceId).first();
            if (accentResult?.value)
                accent = accentResult.value;
            const canvasResult = await c.env.DB.prepare(`SELECT value FROM brand_tokens
         WHERE workspace_id = ? AND category = 'colors' AND key = 'canvas' AND locale = ''`).bind(workspaceId).first();
    ...
    app.get('/_ensemble/brand/css', async (c) => {
        ...
        let etag = '"empty"';
        try {
            const row = await c.env.DB.prepare(`SELECT MAX(updated_at) AS latest FROM brand_tokens WHERE workspace_id = ?`).bind(workspaceId).first();
            if (row?.latest)
                etag = `"${row.latest.replace(/[^0-9]/g, '')}"`;
        }
        catch {
    ...
    app.put('/_ensemble/brand/tokens', async (c) => {
        try {
           ...
           DO UPDATE SET value = excluded.value, updated_at = datetime('now')`).bind(workspace.id, body.category, key, value, tokenType).run();
            }
...
// ============================================================================
function generateShellHtml(workspaceName, accentColor) {
    return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
...
function generateLoginHtml(workspaceName, accentColor) {
    return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
...
//# sourceMappingURL=create-workspace-v2.js.map