...
export function registerBrandRoutes(app) {
    ...
    const specHandler = async (c) => {
        ...
        if (!isAdminPath) {
            ...
            if (!(await isPublicBrandEnabled(c.env, workspace.id)))
                return c.notFound();
        }
    ...
    // follows the toggle.
    const schemaHandler = async (c) => {
        const workspace = c.get('workspace');
        if (!workspace?.id)
            return c.notFound();
        const isAdminPath = c.req.path.startsWith('/_ensemble/');
        if (!isAdminPath && !(await isPublicBrandEnabled(c.env, workspace.id)))
            return c.notFound();
    ...
    const changelogHandler = async (c) => {
        const workspace = c.get('workspace');
        if (!workspace?.id)
            return c.notFound();
        const isAdminPath = c.req.path.startsWith('/_ensemble/');
        if (!isAdminPath && !(await isPublicBrandEnabled(c.env, workspace.id)))
            return c.notFound();
        try {
            const rows = await c.env.DB.prepare(`SELECT action, created_at, details
           FROM audit_log
