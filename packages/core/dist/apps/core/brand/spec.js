...
assetAliasPath) {
    ...
    // ── v1.1: Workspace meta ──
    const workspaceRow = await db.prepare(`SELECT id, slug, display_name FROM workspaces WHERE id = ?`).bind(workspaceId).first();
...
async function assembleSpatialV11(db, workspaceId) {
    ...
    const hasV10Spatial = tokens['radius'] !== undefined ||
