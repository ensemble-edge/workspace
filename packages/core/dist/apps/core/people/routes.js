...
export function registerPeopleRoutes(app) {
    ...
    app.get('/_ensemble/core/people/members', async (c) => {
        ...
        try {
         ...
         ORDER BY m.role ASC, u.display_name ASC`).bind(workspace.id).all();
    ...
    app.post('/_ensemble/core/people/invite', async (c) => {
        ...
        try {
            await c.env.DB.prepare(`INSERT INTO users (id, email, password_hash, display_name, locale)
         VALUES (?, ?, ?, ?, 'en')`).bind(userId, body.email, passwordHash, body.displayName || body.email.split('@')[0]).run();
