...
import { buildLegalSeedStatements } from './seed.js';
...
export function registerLegalRoutes(app) {
    ...
    app.put('/_ensemble/core/legal/docs/:id', async (c) => {
        ...
        const existing = await c.env.DB.prepare(`SELECT id, slugs_json, title_json, description_json, notice_json, body_md_json,
              last_updated, status, sort_order
         FROM legal_docs WHERE workspace_id = ? AND id = ?`)
            .bind(workspace.id, id)
            .first();
        ...
        const noticeJson = body.notice === undefined
        ...
        if (existing) {
           ...
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(workspace.id, id, existing.slugs_json, existing.title_json, existing.description_json, existing.notice_json, existing.body_md_json, existing.last_updated, existing.status, who, now));
        }
        // 2. UPSERT legal_docs.
        stmts.push(c.env.DB.prepare(`INSERT INTO legal_docs
         ...
         ON CONFLICT (workspace_id, id) DO UPDATE SET
           ...
           updated_at = excluded.updated_at`).bind(workspace.id, id, JSON.stringify(body.slugs), JSON.stringify(body.title), descJson, noticeJson, JSON.stringify(body.bodyMd), body.lastUpdated, status, sortOrder, who, now, who, now));
