...
import { setAccessTokenCookie, setRefreshTokenCookie, getCookieOptionsForEnv, } from '../utils/cookies.js';
import { signAccessToken, signRefreshToken, getJwtSecret } from '../utils/jwt.js';
import { markBootstrapComplete } from '../middleware/bootstrap.js';
import { buildLegalSeedStatements } from '../apps/core/legal/seed.js';
import { buildInstalledAppsSeed } from '../services/app-registry.js';
/**
 ...
 * @returns Hono router
 */
export function createBootstrapRoutes(config) {
    ...
    bootstrap.post('/', async (c) => {
        ...
        try {
            ...
            await c.env.DB.batch([
                // Create workspace
                c.env.DB.prepare(`INSERT INTO workspaces (id, slug, name, type, settings_json, created_at, updated_at)
           VALUES (?, ?, ?, 'organization', '{}', ?, ?)`).bind(workspaceId, config.workspace.slug, workspaceName, now, now),
                // Create user
                c.env.DB.prepare(`INSERT INTO users (id, email, password_hash, handle, display_name, locale, created_at)
           VALUES (?, ?, ?, ?, ?, 'en', ?)`).bind(userId, email, passwordHash, handle, displayName, now),
                // Create membership (owner role)
                c.env.DB.prepare(`INSERT INTO memberships (user_id, workspace_id, role, created_at)
           VALUES (?, ?, 'owner', ?)`).bind(userId, workspaceId, now),
                // Seed default brand tokens
                c.env.DB.prepare(`INSERT INTO brand_tokens (workspace_id, category, key, value, type, label, updated_at)
           VALUES (?, 'colors', 'accent', ?, 'color', 'Accent', ?)`).bind(workspaceId, config.brand.accent, now),
                c.env.DB.prepare(`INSERT INTO brand_tokens (workspace_id, category, key, value, type, label, updated_at)
           VALUES (?, 'colors', 'primary', '#1a1a2e', 'color', 'Primary', ?)`).bind(workspaceId, now),
                c.env.DB.prepare(`INSERT INTO brand_tokens (workspace_id, category, key, value, type, label, updated_at)
           VALUES (?, 'colors', 'surface', '#ffffff', 'color', 'Surface', ?)`).bind(workspaceId, now),
                c.env.DB.prepare(`INSERT INTO brand_tokens (workspace_id, category, key, value, type, label, updated_at)
           VALUES (?, 'colors', 'background', '#fafafa', 'color', 'Background', ?)`).bind(workspaceId, now),
                c.env.DB.prepare(`INSERT INTO brand_tokens (workspace_id, category, key, value, type, label, updated_at)
           VALUES (?, 'identity', 'display_name', ?, 'text', 'Display Name', ?)`).bind(workspaceId, workspaceName, now),
                // Seed default nav config
                c.env.DB.prepare(`INSERT INTO nav_config (workspace_id, config_json, updated_at)
           VALUES (?, ?, ?)`).bind(workspaceId, JSON.stringify(getDefaultNavConfig()), now),
                // Create audit log entry
                c.env.DB.prepare(`INSERT INTO audit_log (id, workspace_id, actor_id, actor_handle, action, resource_type, resource_id, details_json, created_at)
           VALUES (?, ?, ?, ?, 'workspace.bootstrapped', 'workspace', ?, '{}', ?)`).bind(generateId('audit'), workspaceId, userId, handle, workspaceId, now),
            ...
            await c.env.DB.prepare(`INSERT INTO sessions (id, user_id, workspace_id, token_hash, expires_at, created_at)
         VALUES (?, ?, ?, ?, ?, ?)`).bind(sessionId, userId, workspaceId, tokenHash, expiresAt, now).run();
            // Set cookies
            ...
            return c.html(generateBootstrapHtml(config, { form: errorMessage }, formData), 500);
        }
    });
    return bootstrap;
}
 ...
 */
function generateBootstrapHtml(config, errors, values) {
    const errorHtml = (field) => {
        if (errors[field]) {
