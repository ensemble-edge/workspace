...
import { renderMagicLinkEmail, renderInviteEmail, renderPasswordResetEmail, } from '../services/email-templates.js';
import { listTiers, getTier, createTier, patchTier, deleteTier, provisionTierRoute, seedDefaultTiers, canaryForProvider, gatewayDashboardUrl, } from '../services/ai-tiers.js';
import { listLocales, addLocale, patchLocale, setDefaultLocale, removeLocale, countLocalizedBrandTokens, } from '../services/locales.js';
import { getSetting, setSetting, parseSessionTtl, SESSION_TTL_OPTIONS, validateAliasPath, } from '../services/workspace-settings.js';
import { getR2Bucket } from '../services/r2-binding.js';
...
export function createCredentialsRoutes() {
    ...
    async function nextVersion(r2, prefix) {
    ...
    app.post('/_ensemble/brand/upload', async (c) => {
        ...
        const kindRaw = form.get('kind') || 'logo';
        ...
        if (SVG_REQUIRED_ROLES.has(role) && file.type !== 'image/svg+xml') {
            return c.json({
                ...
                detail: `The ${role} slot requires an SVG file. We generate PNG, JPG, ` +
    ...
    app.get('/_ensemble/core/brand/logo-policy', async (c) => {
        ...
        const rows = await c.env.DB.prepare(`SELECT key, value FROM brand_tokens
       WHERE workspace_id = ? AND category = 'colors' AND locale = ''
         AND key IN ('brand-primary', 'brand-background-light', 'brand-background-dark')`).bind(workspace.id).all();
        const colors = { bgLight: '#ffffff', bgDark: '#0a0a0a', primary: '#3b82f6' };
        for (const r of rows.results ?? []) {
            if (r.key === 'brand-primary')
                colors.primary = r.value;
            else if (r.key === 'brand-background-light')
                colors.bgLight = r.value;
            else if (r.key === 'brand-background-dark')
                colors.bgDark = r.value;
        }
    ...
    app.put('/_ensemble/core/brand/logo-policy', async (c) => {
        ...
        const { savePolicy, defaultPolicy } = await import('../services/brand-policy.js');
    ...
    app.post('/_ensemble/core/brand/typography/save', async (c) => {
        ...
        const collect = (tokens, prefixes) => {
            for (const p of prefixes) {
                ...
                if (pairs.some((q) => q.family === family && q.weight === weight))
                    continue;
        ...
        const { installFontIfMissing } = await import('../services/brand-render/install-font.js');
        const installed = [];
        ...
        for (const [k, v] of Object.entries(typo))
            upserts.push({ category: 'typography', key: k, value: v });
        for (const [k, v] of Object.entries(ident))
            upserts.push({ category: 'identity', key: k, value: v });
        // Best-effort batch. Empty values become deletes (operator
        // cleared a role).
        for (const row of upserts) {
            if (row.value === '') {
                await c.env.DB.prepare(`DELETE FROM brand_tokens WHERE workspace_id = ? AND category = ? AND key = ? AND locale = ''`).bind(workspace.id, row.category, row.key).run();
            }
            else {
           ...
           DO UPDATE SET value = excluded.value, updated_at = datetime('now')`).bind(workspace.id, row.category, row.key, row.value).run();
            }
        }
     ...
     */
    async function handleBrandRender(c, composition, finish, backgroundId, options) {
        const workspace = c.get('workspace');
        if (!workspace?.id)
            return c.json({ error: 'workspace not resolved' }, 400);
        ...
        const { renderBrandAssetV2 } = await import('../services/brand-render/render.js');
    ...
    app.get('/_ensemble/core/brand/render', async (c) => {
        const composition = (c.req.query('composition') || 'wordmark-only');
        const finish = (c.req.query('finish') || 'full-color');
        ...
        const overrides = {
            ...
            iconSide: c.req.query('iconSide'),
            iconPosition: c.req.query('iconPosition'),
    ...
    app.get('/_ensemble/brand/favicon.svg', async (c) => {
        ...
        if (!workspace?.id)
            return c.notFound();
        const { getIconSvg } = await import('../services/brand-render/sources.js');
     ...
     * busting.
     */
    async function renderFaviconPng(c, sizePx) {
        const workspace = c.get('workspace');
        if (!workspace?.id)
            return c.notFound();
        const { renderBrandAssetV2 } = await import('../services/brand-render/render.js');
    ...
    app.get('/_ensemble/brand/favicon.ico', async (c) => {
        ...
        const { renderBrandAssetV2 } = await import('../services/brand-render/render.js');
        const { wrapPngInIco } = await import('../services/brand-render/favicon.js');
        const result = await renderBrandAssetV2({
            env: c.env,
            workspaceId: workspace.id,
            workspaceSlug: (workspace.slug || workspace.id).toLowerCase(),
        ...
        // array view.
        const icoBuf = ico.buffer.slice(ico.byteOffset, ico.byteOffset + ico.byteLength);
    ...
    app.get('/_ensemble/brand/manifest.webmanifest', async (c) => {
        ...
        if (!workspace?.id)
            return c.notFound();
        const { buildWebManifest } = await import('../services/brand-render/favicon.js');
        const { getSetting } = await import('../services/workspace-settings.js');
        ...
        const idRows = await c.env.DB.prepare(`SELECT key, value FROM brand_tokens
       WHERE workspace_id = ? AND category = 'identity' AND locale = ''
         AND key IN ('display_name', 'workspace_name')`).bind(workspace.id).all();
        const idTokens = {};
        for (const r of idRows.results ?? [])
            idTokens[r.key] = r.value;
    ...
    app.put('/_ensemble/credentials/:key', async (c) => {
        ...
        const { recordAudit, auditContext } = await import('../services/audit-log.js');
    ...
    app.post('/_ensemble/credentials/test/connection', async (c) => {
        ...
        if (zonesR.ok) {
            const zonesBody = await zonesR.json();
            const probeZone = zonesBody.result?.[0];
            if (probeZone) {
                const dnsR = await fetch(`https://api.cloudflare.com/client/v4/zones/${probeZone.id}/dns_records?per_page=1`, { headers: auth });
                scopes.push({
                    name: 'Zone — DNS:Read',
                    ok: dnsR.ok,
                    detail: dnsR.ok
        ...
        if (accountId) {
            ...
            if (gatewayName) {
                const runR = await fetch(`https://api.cloudflare.com/client/v4/accounts/${accountId}/ai-gateway/gateways/${gatewayName}/logs?per_page=1`, { headers: auth });
                scopes.push({
                    name: 'AI Gateway:Run',
                    ok: runR.ok,
                    detail: runR.ok
        ...
        // including this.
        if (accountId) {
            const waiR = await fetch(`https://api.cloudflare.com/client/v4/accounts/${accountId}/ai/models?per_page=1`, { headers: auth });
            scopes.push({
        ...
        if (accountId) {
            const esR = await fetch(`https://api.cloudflare.com/client/v4/accounts/${accountId}/email-security/settings`, { headers: auth });
    ...
    app.put('/_ensemble/credentials/r2/binding-name', async (c) => {
        ...
        if (!name)
            return c.json({ error: 'Binding name cannot be empty' }, 400);
    ...
    app.post('/_ensemble/ai/tiers/:name/test', async (c) => {
        ...
        const r = await fetch(`https://gateway.ai.cloudflare.com/v1/${accountId}/${gatewayName}/compat/chat/completions`, {
            method: 'POST',
            headers: {
        ...
        if (!r.ok && r.status === 400) {
            ...
            if (internalCode === 2005) {
                ...
                try {
                    const routesR = await fetch(`https://api.cloudflare.com/client/v4/accounts/${accountId}/ai-gateway/gateways/${gatewayName}/routes`, { headers: { 'Authorization': `Bearer ${cfToken}` } });
                    if (routesR.ok) {
                        const routesBody = await routesR.json();
                        const routeMeta = routesBody.data?.routes?.find((x) => x.name === tier.gateway_route);
                        if (routeMeta) {
                            const detailR = await fetch(`https://api.cloudflare.com/client/v4/accounts/${accountId}/ai-gateway/gateways/${gatewayName}/routes/${routeMeta.id}`, { headers: { 'Authorization': `Bearer ${cfToken}` } });
                            if (detailR.ok) {
    ...
    app.post('/_ensemble/ai/call/:tier', async (c) => {
        ...
        const incomingBody = await c.req.json().catch(() => null);
    ...
    function aliasTo(canonicalPath) {
        return (c) => app.fetch(new Request(`${new URL(c.req.url).origin}${canonicalPath}${new URL(c.req.url).search}`, c.req.raw), c.env, c.executionCtx);
    }
    ...
    app.get('/brand/render/:filename{.+}', (c) => {
        ...
        return app.fetch(new Request(`${new URL(c.req.url).origin}/_ensemble/brand/render/${filename}${search}`, c.req.raw), c.env, c.executionCtx);
    });
    // R2 asset reads — same handler, short path.
    app.get('/brand/asset/:key{.+}', (c) => {
        const key = c.req.param('key');
        return app.fetch(new Request(`${new URL(c.req.url).origin}/_ensemble/brand/asset/${key}`, c.req.raw), c.env, c.executionCtx);
    });
    ...
    app.put('/_ensemble/admin/brand/tokens', (c) => app.fetch(new Request(`${new URL(c.req.url).origin}/_ensemble/brand/tokens`, c.req.raw), c.env, c.executionCtx));
    app.post('/_ensemble/admin/brand/upload', (c) => app.fetch(new Request(`${new URL(c.req.url).origin}/_ensemble/brand/upload`, c.req.raw), c.env, c.executionCtx));
 ...
 */
function parseImageWidth(buf, mime) {
