...
export function createCredentialsRoutes() {
    ...
    async function handleBrandRender(c, composition, finish, backgroundId, options) {
        ...
        if (!workspace?.id)
            return c.json({ error: 'workspace not resolved' }, 400);
        const { loadPolicy, isPairAllowed } = await import('../services/brand-policy.js');
        const { renderBrandAsset } = await import('../services/brand-assets.js');
        const policy = await loadPolicy(c.env.DB, workspace.id);
       ...
       WHERE workspace_id = ? AND category = 'colors' AND locale = ''
        ...
        for (const r of rows.results ?? []) {
            if (r.key === 'brand-primary')
                brandColors.primary = r.value;
            else if (r.key === 'brand-background-light')
                brandColors.bgLight = r.value;
            else if (r.key === 'brand-background-dark')
                brandColors.bgDark = r.value;
        }
        ...
        const svg = await renderBrandAsset({ composition, finish, backgroundId, backgrounded: options.backgrounded }, { workspaceId: workspace.id, env: c.env, policy, brandColors });
        if (!svg) {
            return c.json({
                error: 'render_failed',
    ...
    app.get('/_ensemble/brand/favicon.svg', async (c) => {
        ...
        if (!workspace?.id)
            return c.notFound();
        const { getIconSvg } = await import('../services/brand-assets.js');
