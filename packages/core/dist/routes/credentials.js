...
export function createCredentialsRoutes() {
    ...
    // shell back-compat.
    const brandUploadPostHandler = async (c) => {
    ...
    const renderHandler = async (c, next) => {
    ...
    const versionPayload = () => ({
        ...
        version: '0.1.100',
        buildFingerprint: 'v0.1.100-multi-accent-and-migration-005-fix',
    ...
    const faviconSvgHandler = async (c) => {
        const workspace = c.get('workspace');
        if (!workspace?.id)
            return c.notFound();
    ...
    const ogPng404Handler = (c) => c.json({
        error: 'no_og_image',
    ...
    // register directly with the same body — no re-dispatch.
    const faviconIcoHandler = async (c) => {
        const workspace = c.get('workspace');
        if (!workspace?.id)
            return c.notFound();
    ...
    const assetHandler = (c) => serveBrandAsset(c, decodeURIComponent(c.req.param('key')));
