...
export function createCredentialsRoutes() {
    ...
    async function handleBrandRender(c, composition, finish, backgroundId, options) {
       ...
       WHERE workspace_id = ? AND category = 'colors' AND locale = ''
        ...
        const svg = await renderBrandAsset({ composition, finish, backgroundId }, { workspaceId: workspace.id, env: c.env, policy, brandColors });
        if (!svg) {
            return c.json({
                error: 'render_failed',
    ...
    app.get('/brand/:filename{.+}', async (c, next) => {
        ...
        const composition = compRaw === 'wordmark' ? 'wordmark-only'
        ...
        if (compIdx + 3 !== parts.length)
            return c.notFound();
        const finish = `${parts[compIdx + 1]}-${parts[compIdx + 2]}`;
        const KNOWN_FINISHES = new Set(['full-color', 'mono-black', 'mono-white', 'mono-brand']);
        if (!KNOWN_FINISHES.has(finish))
            return c.notFound();
