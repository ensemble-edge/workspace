...
import { setAccessTokenCookie, setRefreshTokenCookie, clearAuthCookies, getAuthCookies, getCookieOptionsForEnv, } from '../utils/cookies.js';
import { getJwtSecret } from '../utils/jwt.js';
import { sendEmail } from '../services/email.js';
import { renderMagicLinkEmail } from '../services/email-templates.js';
import { getCredential, getWorkspacePublicUrl } from '../services/credentials.js';
...
export function createAuthRoutes() {
    ...
    auth.post('/login', async (c) => {
        try {
            ...
            if (message === 'Invalid credentials' || message === 'User is not a member of this workspace') {
                ...
                if (ws?.id) {
                    try {
                        const failBody = await c.req.json().catch(() => ({}));
                        await recordAudit(c.env, {
                            workspaceId: ws.id,
    ...
    auth.post('/magic-link', async (c) => {
        try {
            ...
            const body = await c.req.json();
            if (!body.email)
                return c.json({ ok: true }); // generic response
            if (!workspace?.id)
            ...
            if (count >= 3)
                return c.json({ ok: true }); // silently swallow
            ...
            const user = await c.env.DB.prepare(`SELECT u.id, u.email FROM users u
         ...
         WHERE u.email = ? AND m.workspace_id = ?`)
            ...
            await c.env.KV.put(`magic:${token}`, JSON.stringify({ user_id: user.id, workspace_id: workspace.id, created_at: Date.now() }), { expirationTtl: 15 * 60 });
    ...
    auth.get('/magic-link/consume', async (c) => {
        ...
        const row = await c.env.DB.prepare(`SELECT u.id, u.email, u.handle, m.role
         FROM users u
         JOIN memberships m ON m.user_id = u.id
