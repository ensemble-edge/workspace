...
export function createGuestGatewayRoutes() {
    ...
    app.get('/', async (c) => {
        ...
        try {
            const apps = await c.env.DB.prepare(`SELECT id, name, version, category, enabled, connection_type, required_role
         FROM guest_apps
    ...
    app.get('/:appId/manifest', async (c) => {
        ...
        try {
            const app = await c.env.DB.prepare(`SELECT * FROM guest_apps WHERE workspace_id = ? AND id = ?`).bind(workspace.id, appId).first();
            if (!app) {
    ...
    app.all('/:appId/*', async (c) => {
        ...
        const appReg = await c.env.DB.prepare(`SELECT * FROM guest_apps WHERE workspace_id = ? AND id = ?`).bind(workspace.id, appId).first();
        if (!appReg) {
            return c.json({ error: 'App not found' }, 404);
        }
        if (!appReg.enabled) {
        ...
        try {
            ...
            if (appReg.connection_type === 'service_binding' && appReg.binding_name) {
                ...
                response = await proxyViaServiceBinding(c.env, appReg.binding_name, appPath, c.req.raw, contextHeaders);
            }
            else if (appReg.endpoint_url) {
                // Use HTTP fetch (remote services)
                response = await proxyViaHttp(appReg.endpoint_url, appPath, c.req.raw, contextHeaders);
            }
            else {
 ...
 */
async function proxyViaServiceBinding(env, bindingName, path, originalRequest, headers) {
 ...
 */
async function proxyViaHttp(endpointUrl, path, originalRequest, headers) {
 ...
 */
async function generateCapabilityToken(env, payload) {
...
async function logAuditEntry(env, entry) {
    await env.DB.prepare(`INSERT INTO audit_log (id, workspace_id, user_id, action, resource_type, resource_id, details_json, created_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, datetime('now'))`).bind(crypto.randomUUID(), entry.workspace_id, entry.user_id, entry.action, entry.resource_type, entry.resource_id, JSON.stringify(entry.details)).run();
}
//# sourceMappingURL=guest-gateway.js.map