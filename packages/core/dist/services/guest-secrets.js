...
const PURPOSE = 'ensemble:guest-secrets:v1';
/**
 ...
 */
export async function getSecret(env, scope, key) {
    ...
    const row = scope.userId === null
        ? await env.DB.prepare(`SELECT value_encrypted FROM guest_secrets
          WHERE workspace_id = ? AND app_id = ? AND user_id IS NULL AND key = ?`).bind(scope.workspaceId, scope.appId, key).first()
        : await env.DB.prepare(`SELECT value_encrypted FROM guest_secrets
          WHERE workspace_id = ? AND app_id = ? AND user_id = ? AND key = ?`).bind(scope.workspaceId, scope.appId, scope.userId, key).first();
 ...
 */
export async function setSecret(env, scope, key, value, opts = {}) {
    ...
    await env.DB.prepare(`INSERT INTO guest_secrets
     ...
     DO UPDATE SET
       ...
       updated_by_user_id = excluded.updated_by_user_id`).bind(scope.workspaceId, scope.appId, scope.userId, key, encrypted, opts.updatedByUserId ?? null).run();
}
/**
 * Delete a secret. Idempotent — returns true if a row was removed,
 * false if no matching row existed.
 */
export async function deleteSecret(env, scope, key) {
    const r = scope.userId === null
        ? await env.DB.prepare(`DELETE FROM guest_secrets
          WHERE workspace_id = ? AND app_id = ? AND user_id IS NULL AND key = ?`).bind(scope.workspaceId, scope.appId, key).run()
        : await env.DB.prepare(`DELETE FROM guest_secrets
          WHERE workspace_id = ? AND app_id = ? AND user_id = ? AND key = ?`).bind(scope.workspaceId, scope.appId, scope.userId, key).run();
 ...
 * exposing values.
 */
export async function listSecrets(env, scope) {
    let query;
    const params = [scope.workspaceId, scope.appId];
    if (scope.userId === 'any') {
...
//# sourceMappingURL=guest-secrets.js.map