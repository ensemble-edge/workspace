 ...
 * migration script.
 */
export async function migrateLegacyGatewayRoutes(env, workspaceId) {
    await env.DB.prepare(`UPDATE workspace_ai_tiers
        SET gateway_route = REPLACE(gateway_route, 'ws/', 'ws-')
      WHERE workspace_id = ? AND gateway_route LIKE 'ws/%'`).bind(workspaceId).run();
}
export async function seedDefaultTiers(env, workspaceId) {
    for (const t of DEFAULT_TIERS) {
        await env.DB.prepare(`INSERT OR IGNORE INTO workspace_ai_tiers
         (workspace_id, name, display_name, description, icon, is_default, gateway_route, route_provisioned)
       VALUES (?, ?, ?, ?, ?, 1, ?, 0)`).bind(workspaceId, t.name, t.display_name, t.description, t.icon, `ws-${t.name}`).run();
    }
}
export async function listTiers(env, workspaceId) {
    await migrateLegacyGatewayRoutes(env, workspaceId);
    const result = await env.DB.prepare(`SELECT name, display_name, description, icon, is_default,
            gateway_route, route_provisioned, last_error, provider, created_at
       FROM workspace_ai_tiers
      WHERE workspace_id = ?
      ORDER BY is_default DESC, created_at ASC`).bind(workspaceId).all();
    return (result.results ?? []).map(rowToTier);
}
export async function getTier(env, workspaceId, name) {
    await migrateLegacyGatewayRoutes(env, workspaceId);
    const row = await env.DB.prepare(`SELECT name, display_name, description, icon, is_default,
            gateway_route, route_provisioned, last_error, provider, created_at
       FROM workspace_ai_tiers
      WHERE workspace_id = ? AND name = ?`).bind(workspaceId, name).first();
    return row ? rowToTier(row) : null;
}
export async function createTier(env, workspaceId, input) {
    ...
    const provider = input.provider && TIER_PROVIDERS.includes(input.provider) ? input.provider : 'custom';
    await env.DB.prepare(`INSERT INTO workspace_ai_tiers
       (workspace_id, name, display_name, description, icon, is_default, gateway_route, route_provisioned, provider)
     VALUES (?, ?, ?, ?, ?, 0, ?, 0, ?)`).bind(workspaceId, input.name, input.display_name ?? input.name, input.description ?? null, input.icon ?? 'sparkles', `ws-${input.name}`, provider).run();
    const created = await getTier(env, workspaceId, input.name);
    if (!created)
        throw new Error('Failed to read back created tier');
    return created;
}
export async function patchTier(env, workspaceId, name, patch) {
    ...
    await env.DB.prepare(`UPDATE workspace_ai_tiers SET ${fields.join(', ')} WHERE workspace_id = ? AND name = ?`).bind(...values).run();
}
/**
 * Delete a custom tier. Default tiers cannot be deleted.
 * The gateway route is preserved (operator may delete manually in CF).
 */
export async function deleteTier(env, workspaceId, name) {
    ...
    await env.DB.prepare(`DELETE FROM workspace_ai_tiers WHERE workspace_id = ? AND name = ?`).bind(workspaceId, name).run();
}
/**
 ...
 */
function defaultElementsForProvider(provider) {
    const modelPropsByProvider = {
...
export async function provisionTierRoute(env, workspaceId, tierName) {
    const tier = await getTier(env, workspaceId, tierName);
    if (!tier)
        return { ok: false, status: 404, message: `Tier "${tierName}" not found in workspace` };
    ...
    const r = await fetch(`https://api.cloudflare.com/client/v4/accounts/${accountId}/ai-gateway/gateways/${gatewayName}/routes`, {
        method: 'POST',
        headers: {
    ...
    if (r.ok || r.status === 409) {
        ...
        WHERE workspace_id = ? AND name = ?`).bind(workspaceId, tierName).run();
        return { ok: true };
    }
    let body = '';
    try {
    ...
    await env.DB.prepare(`UPDATE workspace_ai_tiers
      ...
      WHERE workspace_id = ? AND name = ?`).bind(message, workspaceId, tierName).run();
 ...
 */
export function canaryForProvider(provider) {
 ...
 */
export function gatewayDashboardUrl(accountId, gatewayName) {
    return `https://dash.cloudflare.com/${accountId}/ai/ai-gateway/configuration/${gatewayName}`;
}
//# sourceMappingURL=ai-tiers.js.map