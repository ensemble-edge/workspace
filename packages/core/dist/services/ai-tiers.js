 ...
 */
import { getCredential, deleteCredential } from './credentials.js';
/**
 ...
 */
async function lazyDeleteLegacyAiToken(env, workspaceId) {
...
export async function createTier(env, workspaceId, input) {
    ...
    await env.DB.prepare(`INSERT INTO workspace_ai_tiers
       (workspace_id, name, display_name, description, icon, is_default, gateway_route, route_provisioned)
     VALUES (?, ?, ?, ?, ?, 0, ?, 0)`).bind(workspaceId, input.name, input.display_name ?? input.name, input.description ?? null, input.icon ?? 'sparkles', `ws/${input.name}`).run();
