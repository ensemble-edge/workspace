 ...
 */
import { resolvePalettes, resolveStopValue, onColorForeground } from './resolver.js';
 ...
 */
export function renderBrandColorsHtml(doc) {
    ...
    const paletteCard = (label, // display name (e.g. "Curalisto Coral")
    roleLabel, // small badge text (e.g. "accent", "accent-2")
    rungLabel, // main rung token shown bottom-right (e.g. "accent-main", "accent-2-main")
    r, fg) => `
      <div style="border-radius:14px;overflow:hidden;background:#fff;border:0.5px solid rgba(0,0,0,0.07);">
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(64px,1fr));gap:8px;padding:12px;background:#fff;">
          ${['dark', 'bright', 'pastel', 'faded'].map((rung) => `
    ...
    const neutralStrip = () => {
      ...
      <div style="border-radius:14px;border:0.5px solid rgba(0,0,0,0.07);background:#fff;padding:20px;display:flex;flex-direction:column;gap:18px;">
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(64px,1fr));gap:8px;">
          ${['dark', 'main', 'bright', 'pastel', 'faded'].map((rung) => {
  ...
  `;
    ...
    <div style="display:flex;flex-direction:column;gap:40px;">
      <section>
        ...
        return paletteCard(stored?.name ?? `Accent ${idx}`, `accent-${idx}`, `accent-${idx}-main`, p, fg);
    }).join('')}
        </div>
      </section>
