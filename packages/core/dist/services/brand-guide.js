...
export async function renderBrandGuide(env, workspaceId) {
    ...
    const { hashSnapshot } = await import('./brand-render/cache.js');
    ...
    <div class="container">
      ...
      <section>
        ...
        const orderedBgs = order
            ...
            .filter((b) => !!b);
        return orderedBgs.map((bg) => `<button type="button" class="filter-chip" data-filter="bg" data-value="${escapeAttr(bg.id)}">${escapeHtml(bg.label)}</button>`).join('');
    })()}
          </div>
          <div class="filter-row">
        ...
        return order
          ...
          <div class="filter-row">
            ...
            ${approvedFinishes.map((f) => `<button type="button" class="filter-chip" data-filter="finish" data-value="${escapeAttr(f.id)}">${escapeHtml(f.label)}</button>`).join('')}
          </div>
        </div>
        ${(() => {
        ...
        return orderedBgs.map((bg) => `
            ...
            <div class="logo-grid">
              ${orderedComps.flatMap((composition) => approvedFinishes.map((finish) => {
            if (banSet.has(`${finish.id}|${bg.id}`))
                return '';
            const compShort = composition === 'wordmark-only' ? 'wordmark'
                : composition === 'icon-only' ? 'icon'
                    : composition;
            const svgUrl = (applyAssetAlias(`/_ensemble/brand/render/${brand.workspace_slug}-${compShort}-${finish.id}-${bg.id}.svg`, aliasPath) ?? '') + `?v=${brandSnapshot}`;
            const pngUrl = (applyAssetAlias(`/_ensemble/brand/render/${brand.workspace_slug}-${compShort}-${finish.id}-${bg.id}.png`, aliasPath) ?? '') + `?v=${brandSnapshot}`;
