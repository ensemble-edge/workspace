 ...
 */
export function renderWordmarkHtml(segments, opts = {}) {
    ...
    const inner = segments
        .map((s) => `<span${s.color ? ` style="color:${escapeAttr(s.color)}"` : ''}>${escapeText(s.text)}</span>`)
        .join('');
    const styles = [
    ...
    if (fontFamily)
        styles.unshift(`font-family:${fontFamily}`);
