...
export function connectToHost() {
    ...
    window.addEventListener('message', (event) => {
        ...
        if (!msg || typeof msg.type !== 'string' || msg.v !== 1)
            return;
        switch (msg.type) {
    ...
    return {
        ...
        audit: (event, details) => send({ type: 'ensemble:audit', v: 1, event, details }),
        resize: (heightPx) => send({ type: 'ensemble:resize', v: 1, height: Math.round(heightPx) }),
        onContext: (cb) => {
