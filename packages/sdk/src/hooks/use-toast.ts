...
type EnsembleToast = (
  ((kind: ToastKind, message: string, options?: ToastOptions) => unknown)
  & {
...
function getEnsembleToast(): EnsembleToast {
  if (typeof window === 'undefined') return undefined;
  const ens = (window as unknown as { Ensemble?: { toast?: EnsembleToast } }).Ensemble;
  return ens?.toast;
}

function dispatch(kind: ToastKind, message: string, options?: ToastOptions): void {
...
export const toast: ToastApi = Object.assign(
  (kind: ToastKind, message: string, options?: ToastOptions) => dispatch(kind, message, options),
  {
