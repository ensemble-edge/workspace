...
class WorkspaceContextClient {
  ...
  private pendingFetch: Promise<WorkspaceContext | null> | null = null;
   ...
   * locale).
   */
  async get(options: { refresh?: boolean } = {}): Promise<WorkspaceContext | null> {
    if (this.ctx && !options.refresh) return this.ctx;
    if (this.pendingFetch && !options.refresh) return this.pendingFetch;
   ...
   */
  async setUserLocale(locale: string | null): Promise<void> {
  ...
  private async fetch(): Promise<WorkspaceContext | null> {
    try {
  ...
  private notifyAll(): void {
    for (const sub of this.subscribers) sub(this.ctx);
  }
}
